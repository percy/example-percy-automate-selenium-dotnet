...
|---|---|---|
| `PercyTest.cs` (basic) | Minimum viable: `Percy.Screenshot(driver, name)` calls. | `dotnet test` |
| [`advanced/`](./advanced) | Full applicable Percy on Automate feature surface: ignore/consider regions, freeze_animation, percy_css, sync mode, test_case + labels. NUnit + Dictionary<string,object>. See [`advanced/README.md`](./advanced/README.md). | `cd advanced && make test` |
...
Minimum required `@percy/cli` and `@percy/webdriver-utils` version is `1.31.2` for this to work correctly. If you already have `@percy/cli` or `@percy/webdriver-utils` installed please update it to latest or minimum required version.
